...
`dist`를 수동으로 업로드하는 배포에서는 저장소가 배포본보다 앞서 있는 상태가 정상입니다. 이때 새로고침해도 커밋이 그대로이므로, 같은 커밋에 대한 재시도는 6시간 동안 하지 않습니다. 이 장치가 없으면 10분마다 무한 새로고침이 됩니다. 저장소에 더 새로운 커밋이 올라오면 대기 시간과 무관하게 다시 시도합니다.
